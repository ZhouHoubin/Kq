...
import z.houbin.kq.http.LianZhong;
import z.houbin.kq.http.URLParser;
import z.houbin.kq.util.Config;
import z.houbin.kq.util.L;
import z.houbin.kq.util.TextUtils;
import z.houbin.kq.util.Utils;
...
public class Weibo {
    ...
    public void publish(final String content, final String picId, final String visibile, final WeiboCallBack weiboCallBack) {
        WeiboResult result = publish(content, picId, visibile);
        try {
            if (result.json.getInt("ok") == 1) {
                weiboCallBack.onSuccess(result);
            } else {
